...
int main()
{
    ...
    BinTreePointer ARoot, node1, node2, LocPtr, Root;
    BinTreeElementType num1, num2, AnItem;
    int count;
    boolean found = FALSE;
    ...
    while(!found) {
        ...
        RecBSTSearch(ARoot, num1, &found, &LocPtr);

        if(found) {
            RecBSTSearch(ARoot, num2, &found, &LocPtr);
        }
    }

    RecBSTInsert(&node1, num1);
    RecBSTInsert(&node2, num2);

    Root = LCANode(ARoot, node1, node2);

    printf("LCA node of %d and %d is node with element %d", num1, num2, Root->Data);
